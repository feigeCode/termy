...
Build a signed macOS app bundle + signed DMG, with optional notarization.
...
Options:
  ...
  --sign-identity NAME    Developer ID Application identity (required)
  --entitlements PATH     Entitlements plist for app signing (optional)
  ...
  --notary-issuer UUID    App Store Connect issuer ID (UUID)
  --no-notarize           Skip notarization + stapling
  --no-sign-dmg           Skip signing the final DMG
  --no-layout             Skip Finder layout customization
  --help, -h              Show this help message

Environment variable defaults:
  TERMY_SIGN_IDENTITY
  TERMY_NOTARY_PROFILE
  TERMY_NOTARY_KEY
  TERMY_NOTARY_KEY_ID
  TERMY_NOTARY_ISSUER
  TERMY_ENTITLEMENTS
...
[[ -n "$SIGN_IDENTITY" ]] || die "Missing signing identity. Set --sign-identity or TERMY_SIGN_IDENTITY"
...
if [[ "$NOTARIZE" -eq 1 ]]; then
  ...
  if [[ "$HAS_PROFILE" -eq 0 && "$HAS_API_KEY" -eq 0 ]]; then
    die "Missing notarization credentials. Use --notary-profile, or --notary-key + --notary-key-id + --notary-issuer, or pass --no-notarize"
  fi

  if [[ "$HAS_PROFILE" -eq 1 && "$HAS_API_KEY" -eq 1 ]]; then
    die "Use either --notary-profile OR API key flags (--notary-key/--notary-key-id/--notary-issuer), not both"
  fi
fi
...
APP_NAME="Termy"
OS_NAME="macos"
DMG_NAME="${APP_NAME}-${VERSION}-${OS_NAME}-${ARCH}-signed"
...
if [[ -n "$ENTITLEMENTS" ]]; then
  CODESIGN_APP_ARGS+=(--entitlements "$ENTITLEMENTS")
fi
